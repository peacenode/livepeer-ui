...
const firstNames = [
  "Maya", "Owen", "Priya", "Noah", "Avery",
  "Theo", "Sofia", "Eli", "Nia", "Leo",
]
const lastNames = [
  "Chen", "Park", "Shah", "Williams", "Kim",
  "Martinez", "Okafor", "Nguyen", "Patel", "Rivera",
]
...
export function WaitlistPanel({ joinedInitially = false }: { joinedInitially?: boolean }) {
  ...
  return (
    <aside className="dark relative z-10 flex min-h-[calc(100svh-1.5rem)] w-full max-w-sm flex-col overflow-y-auto rounded-2xl border border-white/20 bg-white/[0.055] px-6 py-6 text-foreground shadow-2xl shadow-black/30 backdrop-blur-md backdrop-saturate-150 sm:min-h-[calc(100svh-2rem)] sm:px-8 sm:py-8 md:h-[calc(100svh-2rem)] md:min-h-0">
      ...
      <div className={joined ? "mt-10 max-w-full sm:mt-12" : "mt-16 max-w-full sm:mt-24 md:mt-28"}>
      ...
      <div className={joined ? "pt-10" : "mt-auto pt-14"}>
        {!joined ? (
          <WaitlistSignupForm onJoin={joinWaitlist} />
        ) : (
          <div className="space-y-6">
            <div className="space-y-1">
              <WaitlistStatusCard />
              <WaitlistReferralLink inviteUrl={inviteUrl} />
            </div>
            ...
            <WaitlistLeaderboard leaders={waitlistLeaders} />
          </div>
        )}
      </div>
    </aside>
