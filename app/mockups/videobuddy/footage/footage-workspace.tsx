...
const initialFootage: Clip[] = [
  { id: 3, name: "salt-flats-wind-test-04.mp4", duration: "0:18", uploaded: "Today, 3:08 PM", imageUrl: storyMedia.saltSignal.wide },
  { id: 2, name: "lighthouse-weather-reference.mov", duration: "0:12", uploaded: "Yesterday", imageUrl: storyMedia.blackTide.wide },
  { id: 1, name: "pool-rehearsal-wide.mp4", duration: "0:24", uploaded: "Jul 21", imageUrl: storyMedia.afterHours.wide },
]
    ...
    <main className="flex h-[calc(100dvh-4rem)] flex-col overflow-hidden md:h-dvh">
      <section className="min-h-0 flex-1 overflow-y-auto overscroll-none">
        <div className="mx-auto max-w-6xl px-4 pb-10 sm:px-6">
          <ClipsHeader onUpload={addFootage} />
          <ClipsLibrary
            clips={footage}
            onDownload={(clip) => downloadMedia(clip.imageUrl, clip.name)}
            onDuplicate={(clip) =>
              setFootage((current) => [
                { ...clip, id: Math.max(...current.map((entry) => entry.id)) + 1, name: `Copy of ${clip.name}`, uploaded: "Just now" },
                ...current,
              ])
            }
            onRemove={(clip) => setFootage((current) => current.filter((entry) => entry.id !== clip.id))}
          />
        </div>
      </section>
    </main>
